...
class LoginFormComponent extends Component {
  ...
  getAppVersionStorage = async () => {
    ...
    if (appVersionNew && appVersionNew.version && APP_VERSION_CONST < appVersionNew.version) {
      Alert.alert('Thông báo', 'Có cập nhập mới, vui lòng download và cài đặt lại để sử dụng tốt hơn !', [
        {
          text: 'Huỷ',
          onPress: () => {
            this.setState({ isUpdating: false });
        ...
        },
        {
          text: 'Cập nhập',
          onPress: async () => {
            Linking.openURL(appVersionNew.url)
              .then(r => {
                this.setState({ isUpdating: false });
              })
              .catch(e => {
  ...
  render() {
    ...
    if (this.state.isUpdating) {
      return (
        <View style={{ flex: 1, flexDirection: 'column', alignItems: 'center', alignContent: 'center', backgroundColor: '#2299ec' }}>
          <View>
    ...
    return (
      <KeyboardAwareScrollView style={scrollView}>
        ...
        <View style={loginBox}>{loading ? <LoadingIndicator color="#ffffff" size="large" /> : <BasicFormComponent buttonTitle={'Đăng nhập'} onButtonPress={login} />}</View>
