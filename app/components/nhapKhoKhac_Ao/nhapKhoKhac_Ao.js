...
import styles from './nhapKhoKhac_AoStyles';
import * as _ from 'lodash';
import apiService from '../../services/apiService';
import LoadingIndicator from '../loadingIndicator/loadingIndicator';
import { DON_VI_TINH } from '../../app_const';
...
const TABLE_LIST_VAT_TU_KHO = [
  ...
  { title: 'Trạng thái', key: 'trangThai', width: 100, order: 4 },
  { title: 'Đơn vị tính', key: 'donViTinh', width: 100, order: 5 },
  { title: 'Hạn sử dụng', key: 'hanSuDung', width: 100, order: 4 },
  { title: 'Id phiếu đặt', key: 'idPhieuDatVatTu', width: 100, order: 4 },
  { title: 'Id phiếu nhập', key: 'idPhieuNhapVatTu', width: 100, order: 4 },
]

const TABLE_LIST_VAT_TU_KHO_SCANED = [
  { title: 'Mã Vật Tư', key: 'maVatTu', width: 100, order: 1 },
  { title: 'Mã NCC', key: 'maNCC', width: 100, order: 4 },
  { title: 'BarCode', key: 'barCode', width: 100, order: 2 },
  { title: 'Trạng thái', key: 'trangThai', width: 100, order: 4 },
  { title: 'Đơn vị tính', key: 'donViTinh', width: 100, order: 5 },
  { title: 'Hạn sử dụng', key: 'hanSuDung', width: 100, order: 4 },
  { title: 'Id phiếu đặt', key: 'idPhieuDatVatTu', width: 100, order: 4 },
...
class NhapKhoAo extends React.Component {
  ...
  getListPO = async () => {
    ...
    try {
      ...
      let rs = await apiService.get('/admin/v1/phieu-dat-vat-tu-khac/get-by-ncc', { params });
      if (rs && rs.data && rs.data.items) {
        this.setState({
          ngSelectPO: {
            ...
            items: _.orderBy(rs.data.items, ['id'], ['asc']).map(item => {
              return {
                ...item,
                label: `${item.id} - Mã NCC:${item.maNCC} - Trang Thai:${item.trangThai} `, //  + ' | ' +item.name,
                value: item.id,
              }
            }),
          }
  ...
  getListPhieuNhap = async () => {
    ...
    try {
      ...
      // let rs = await apiService.get('/admin/v1/phieu-nhap-vat-tu-khac/filter', { params });
      let rs = await apiService.admin.phieuNhapVatTuKhac.filter(params);
      if (rs && rs.data && rs.data.items) {
        this.setState({
          ngSelectPhieuNhap: {
            ...
            items: rs.data.items.map(item => {
              return {
                ...item,
                label: `${item.id} - Mã NCC:${item.maNCC} - Nguon:${item.tuNguon}`,
                value: item.id,
              }
            }),
          }
  ...
  getListVatTuKho = async () => {
    ...
    try {
      ...
      let rs = await apiService.get('/admin/v1/vat-tu-khac-kho/filter', { params });
      if (rs && rs.data && rs.data.items) {
        rs.data.items = _.orderBy(rs.data.items, ['id'], ['asc']);
        let listKhoCuon = [];
        let listOptionsSelectMaVatTu = [];
        rs.data.items = rs.data.items.map(r => { // get options MaVatTu
          if(r.maVatTu && listOptionsSelectMaVatTu.includes(r.maVatTu)==false){
            listOptionsSelectMaVatTu.push(r.maVatTu);
          }
          return r;
        });
        this.setState({
          ...
          // },
          ngSelectFilterMaVatTu: {
            ...this.state.ngSelectFilterMaVatTu,
            items: listOptionsSelectMaVatTu.map(r=>{
              return {
                label: r,
                value: r,
              }
  ...
  convertListToDataTable = (listScanedComplete = false) => {
    let { listVatTuKho } = this.state;
    let dataTable = [];

    if(listScanedComplete==='scanCompleted'){
    ...
    if (listScanedComplete === false) {
      listVatTuKho.filter(r => r.isHide !== true && [null,'',undefined].includes(r.barCode)).map(r => {
        let perRow = [];
        let rowKeys = TABLE_LIST_VAT_TU_KHO.map(ite => ite.key);
        for (let k of rowKeys) {
          if(k==='hanSuDung' && r[k]){
            r[k] = r[k].toString().substring(0,10);
          }
          perRow.push(r[k] || '');
        }
        dataTable.push(perRow);
  ...
  setDataListScanComplete = async () => {
    let { listVatTuKho } = this.state;
    this.setState({
      listVatTuKhoScaned: listVatTuKho.filter(r => r.barCode),
      scanStep: '',
    ...
    if (this.state.scanStep === STEP_SCAN_NHAP_KHO_AO) {
      ...
      if (findIndex >= 0) {
        try {
          ...
          let rs = await apiService.post('/admin/v1/vat-tu-khac-kho/nhap/kho-ao', { ...param, });
          if (rs) {
  ...
  exitCameraScanKhoAo = () => {
    let { listVatTuKho } = this.state;
    let listScanning = listVatTuKho.filter(r => !r.barCode);
  ...
  backToListVatTuKho_ToScanContinue = () => {
    let { listVatTuKho } = this.state;
    this.setState({
      listVatTuKho: listVatTuKho.filter(r => {
        return !r.barCode;
      }).map(r => {
        return r;
      }),
  ...
  filterDSVatTuKhac = ()=>{
    let {listVatTuKho, ngSelectDonViTinh: {selectedItem: selectDonViTinh}, ngSelectFilterMaVatTu:{selectedItem: selectMaVatTu}} = this.state;
    listVatTuKho = listVatTuKho.map(r => {
      let isShow = true;
      if(selectDonViTinh){
        isShow = isShow && (r.donViTinh == selectDonViTinh.value);
      }
      if(selectMaVatTu){
        isShow = isShow && (r.maVatTu == selectMaVatTu.value);
      }
      r.isHide = !isShow;
      return r;
    });
  ...
  render() {
    let { ngSelectNhaCC, ngSelectPO, ngSelectPhieuNhap, listVatTuKho, 
      // ngSelectKhoCuon, 
      viewType } = this.state;
    let { listVatTuKhoScaned, searchCondition, ngSelectDonViTinh, ngSelectFilterMaVatTu } = this.state;
    ...
    if (viewType === VIEW_LIST_VAT_TU) {
        ...
        return (
          <View style={{ flex: 1, flexDirection: 'column' }}>
            ...
            <View style={{ position: 'absolute', top: 0, left: 0, width: '100%', alignContent: 'center', alignItems: 'center', zIndex: 9999 }}>
              <Text style={{ color: 'white', fontWeight: 'bold', fontSize: 14, }}>Vui lòng scan barcode nhập v/tư khác vào kho ảo</Text>
            </View>
            ...
            <View style={{ flexDirection: 'row', alignContent: 'flex-start', alignItems: 'center' }}>
              ...
              <TouchableOpacity onPress={() =>{ return totalItemsScan == 0 || totalScanedItems == 0 ? null : this.setState({scanStep:STEP_REVIEW_PROCESS_SCANED}) }} 
      ...
      if(this.state.scanStep === STEP_REVIEW_PROCESS_SCANED){
        ...
        return (
          <ScrollView>
            <View>
              <Text style={{ ...styles.textLabel, textAlign: 'center' }}>Tiến trình d/sách vật tư khác đã scan</Text>
            </View>
            <View>
            ...
            <View>
              <TouchableOpacity onPress={() => {  this.setState({ scanStep: STEP_SCAN_NHAP_KHO_AO, }) }}
      ...
      return (
        <ScrollView>
          <View>
            <Text style={{ ...styles.textLabel, textAlign: 'center' }}>Danh sách vật tư khác trong kho</Text>
          </View>
          <View>
            <Text style={styles.textLabel}>Filter Mã V/tư</Text>
            <RNPickerSelect 
              style={APP_STYLES.ngSelectStyle}
              onValueChange={(e) => {
                console.log(`filter Mã V/tư `, e);
                let item = _.find(ngSelectFilterMaVatTu.items, { value: e });
                ngSelectFilterMaVatTu.selectedItem = item;
                this.setState({ ngSelectFilterMaVatTu},()=>{
                  this.filterDSVatTuKhac();
                });
              }}
              items={ngSelectFilterMaVatTu.items}
            />
          </View>
          <View>
            <Text style={styles.textLabel}>Filter Đ/vị tính</Text>
            <RNPickerSelect 
              style={APP_STYLES.ngSelectStyle}
              onValueChange={(e) => {
                console.log(`Filter Đơn vị tính `, e);
                let item = _.find(ngSelectDonViTinh.items, { value: e });
                ngSelectDonViTinh.selectedItem = item;
                this.setState({ ngSelectDonViTinh},()=>{
                  this.filterDSVatTuKhac();
                });
              }}
              items={ngSelectDonViTinh.items}
            />
          </View>
          <View>
              ...
              <ScrollView horizontal={true} style={{ marginTop: 15 }}>
                <View>
                  ...
                  <ScrollView style={{}}>
                    <Table borderStyle={{ borderWidth: 1, borderColor: '#C1C0B9' }}>
                      {
                        this.convertListToDataTable().map((item, index) => ( //item is array
                          <Row
                            key={index}
                            data={item}
                            widthArr={TABLE_LIST_VAT_TU_KHO.map(r => r.width)}
          ...
          <View>
            {listVatTuKho && listVatTuKho.length > 0
    ...
    if (viewType === VIEW_LIST_VAT_TU_AFTER_SCANED) {
      return (
        <ScrollView>
          <View>
            <Text style={{ ...styles.textLabel, textAlign: 'center' }}>Danh sách v/tư khác sau khi scan xong</Text>
          </View>
          <View>
            {this.state.listVatTuKhoScaned && this.state.listVatTuKhoScaned.length
              ?
              <ScrollView horizontal={true} style={{ marginTop: 15 }}>
                <View>
                  <Table borderStyle={{ borderWidth: 1, borderColor: '#C1C0B9' }}>
                  ...
                  <ScrollView style={{}}>
                    <Table borderStyle={{ borderWidth: 1, borderColor: '#C1C0B9' }}>
                      {
                        this.convertListToDataTable(true).map((item, index) => ( //item is array
                          <Row
                            key={index}
                            data={item}
                            widthArr={TABLE_LIST_VAT_TU_KHO_SCANED.map(r => r.width)}
